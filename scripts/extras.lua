...
local function draw_line(from, to, properties)
    local p = 1
    
    if not from then
        from = UDim2.new(0, 0, 0, 0)
    else
        from = UDim2.new(0, m.X, 0, m.Y)
    end

    if not to then
        to = UDim2.new(0, 0, 0, 0)
    else
        to = UDim2.new(0, n.X, 0, n.Y)
    end
    
    local q, r = to.X.Offset - from.X.Offset, to.Y.Offset - from.Y.Offset
    local h = (q * q + r * r) ^ 0.5
    local s = math.atan2(r, q)
    
    properties.Size = UDim2.new(0, h, 0, p)
    
    local t, u = 0.5 * (from.X.Offset + to.X.Offset), 0.5 * (from.Y.Offset + to.Y.Offset)
    properties.Position = UDim2.new(0, t - 0.5 * h, 0, u - 0.5 * p)
    properties.Rotation = math.deg(s)
...
Drawing.new = newcclosure(function(drawing_type)    
    ...
    if drawing_type == "Circle" then
        local circle_properties = {}
        local circle_frame = Instance.new("Frame", Parent)
        circle_frame.BorderSizePixel = 0
        circle_frame.AnchorPoint = Vector2.new(0.5, 0.5)
        Instance.new("UICorner", circle_frame).CornerRadius = UDim.new(1, 0)

        local stroke = Instance.new("UIStroke", circle_frame)
        stroke.Thickness = 0.5
        stroke.Color = Color3.new(0, 0, 0)
        stroke.Enabled = false
        ...
        return setmetatable({}, {
            __index = function(self, x)
                ...
                return circle_properties[x]
            end,
            __newindex = function(self, x, y)
                circle_properties[x] = y
                if x == "Visible" then
                circle_frame.Visible = y
                end
                if x == "Filled" then
                circle_frame.BackgroundTransparency = y == true and 0 or 1
                  stroke.Enabled = not y
                end
                if x == "Transparency" then
                    circle_frame.BackgroundTransparency = 1 - y
                end
                if x == "Color" then
                    circle_frame.BackgroundColor3 = y
                    stroke.Color = y
                end
                if x == "Position" then
                    circle_frame.Position = UDim2.new(0, y.X, 0, y.Y)
                end
                if x == "Radius" then
                    circle_frame.Size = UDim2.new(0, y * 2, 0, y * 2)
                end
            end
        })
    end

    if drawing_type == "Square" then
        local E = {}
