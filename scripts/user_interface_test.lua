		...
		do
			...
			container.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
					local inputConn = userInputService.InputChanged:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
							local percentage = (input.Position.X - container.AbsolutePosition.X) / container.AbsoluteSize.X;
							tween(indicator, 0.18, {
								Size = UDim2.new(percentage, 0, 1, 0)
							});
							local value = math.clamp(math.round(1 + (998 * percentage)), 1, 999);
